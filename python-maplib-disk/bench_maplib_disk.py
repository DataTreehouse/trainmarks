...
def bench_io(scale, ttl_path, nt_path):
    ...
    def read_ttl():
        ...
        return m
    m, t, mem = timed("Read Turtle (parallel=True, disk)", read_ttl, timeout=TIMEOUT)
    if t is not None:
    ...
    def read_nt():
        ...
        return m2
    _, t, mem = timed("Read N-Triples (parallel=True, disk)", read_nt, timeout=TIMEOUT)
    if t is not None:
